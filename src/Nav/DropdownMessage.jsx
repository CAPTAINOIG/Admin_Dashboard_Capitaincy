    ...
    return (
        ...
        <div className="relative dark:text-gray-700" x-data="{ dropdownOpen: false, notifying: true }">
                <Link ref={trigger} onClick={() => setDropdownOpen(!dropdownOpen)}>
                    <BsChatDots size={27} className='cursor-pointer text-gray-500 bg-gray-300 rounded-full p-1 dark:text-gray-800 font-medium' />
                </Link>
                <div ref={dropdown} onFocus={() => setDropdownOpen(true)} onBlur={() => setDropdownOpen(false)} className={`absolute md:-right-16 right-0 mt-2.5 flex lg:h-96 h-60 flex-col lg:w-[19rem] w-[15rem] border border-stroke text-sm font-serif bg-gray-700 text-white dark:bg-gray-700 dark:border-strokedark dark:bg-boxdark sm:right-0 sm:w-80 px-3 rounded ${dropdownOpen === true ? 'block' : 'hidden'}`}>
                    <div className='px-4'>
                        <p className='font-serif lg:text-2xl text-center font-medium text-black dark:text-white'>Messages</p>
                    </div>
